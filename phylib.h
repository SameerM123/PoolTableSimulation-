...
#define PHYLIB_HOLE_RADIUS (2*(PHYLIB_BALL_DIAMETER)) // mm
...
#define PHYLIB_DRAG     (150.0)  //accel, mm/s^2
#define PHYLIB_MAX_TIME  (600)  //s

#define PHYLIB_MAX_OBJECTS    (26)

typedef enum {                //declares all objects
    PHYLIB_STILL_BALL = 0,
...
typedef struct {         //coordinates of ball, regardless of whether it moving or not
  double x;
  double y;
} phylib_coord;


typedef struct{           //still ball profile
    unsigned char number;
    phylib_coord pos; 
}phylib_still_ball;

typedef struct{           //things governing rolling ball and its physics
    unsigned char number;
    phylib_coord pos;
    phylib_coord vel;
    phylib_coord acc;
...
typedef struct {         //typedef hole, only concerns the hole's position
  phylib_coord pos;
} phylib_hole;
